...
pub fn run_doctor() -> DoctorReport {
    ...
    let agents = vec![
        ...
        check_agent(
            ...
            Some(".kiro/steering/codebase-memory.md"),
            &["Kiro"],
            &["kiro-cli", "kiro"],
        ),
...
fn check_agent(
    ...
    let has_instructions = instr_path.as_ref().is_some_and(|p| {
        p.exists() && std::fs::read_to_string(p).is_ok_and(|c| c.contains("codryn"))
    });
...
fn has_codryn_entry(path: &Path) -> bool {
    std::fs::read_to_string(path).is_ok_and(|c| c.contains("codryn"))
}
