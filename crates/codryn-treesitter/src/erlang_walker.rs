//! Erlang regex-fallback walker.

use crate::{TsParam, TsSymbol};
use regex::Regex;
use std::sync::LazyLock;
use tree_sitter::Tree;

static RE_MODULE: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"-module\s*\(\s*(\w+)\s*\)").unwrap());
static RE_RECORD: LazyLock<Regex> = LazyLock::new(|| Regex::new(r"-record\s*\(\s*(\w+)").unwrap());
static RE_FUNC: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"^(\w+)\s*\(([^)]*)\)\s*->").unwrap());
static RE_EXPORT: LazyLock<Regex> =
    LazyLock::new(|| Regex::new(r"-export\s*\(\s*\[([^\]]*)\]").unwrap());

pub fn walk_tree(_tree: &Tree, source: &str) -> Vec<TsSymbol> {
    let mut symbols = Vec::new();
    // Collect exported function names
    let mut exported: std::collections::HashSet<String> = std::collections::HashSet::new();
    for line in source.lines() {
        if let Some(cap) = RE_EXPORT.captures(line) {
            for item in cap[1].split(',') {
                if let Some(name) = item.split('/').next() {
                    exported.insert(name.trim().to_string());
                }
            }
        }
    }
    let mut seen_funcs: std::collections::HashSet<String> = std::collections::HashSet::new();
    for (i, line) in source.lines().enumerate() {
        let ln = (i as i32) + 1;
        if let Some(cap) = RE_MODULE.captures(line) {
            symbols.push(sym(cap[1].to_string(), "Module", ln, vec![], true));
        }
        if let Some(cap) = RE_RECORD.captures(line) {
            symbols.push(sym(cap[1].to_string(), "Class", ln, vec![], true));
        }
        if let Some(cap) = RE_FUNC.captures(line) {
            let name = cap[1].to_string();
            if !seen_funcs.contains(&name) {
                seen_funcs.insert(name.clone());
                let params = parse_params(&cap[2]);
                let is_exported = exported.contains(&name);
                symbols.push(sym(name, "Function", ln, params, is_exported));
            }
        }
    }
    symbols
}

fn parse_params(s: &str) -> Vec<TsParam> {
    s.split(',')
        .filter_map(|p| {
            let name = p.trim().to_string();
            if name.is_empty() {
                None
            } else {
                Some(TsParam {
                    name,
                    type_name: None,
                })
            }
        })
        .collect()
}

fn sym(name: String, label: &str, line: i32, params: Vec<TsParam>, exported: bool) -> TsSymbol {
    TsSymbol {
        name,
        label: label.into(),
        start_line: line,
        end_line: line,
        parent_name: None,
        signature: None,
        return_type: None,
        parameters: params,
        docstring: None,
        decorators: vec![],
        base_classes: vec![],
        is_exported: exported,
        is_abstract: false,
        is_async: false,
        is_test: false,
        is_entry_point: false,
        body_text: None,
    }
}
